...
class CPSearch(UJSSearch):
    ...
    def search_results_from_page(self, page: str) -> List[SearchResult]:
        ...
        filing_dates = results_panel.xpath(".//span[contains(@id, 'filingDateLabel')]")
        ...
        docket_sheet_urls = results_panel.xpath(
            ".//a[contains(@href,'CPReport.ashx?docketNumber=')]"
        )

        summary_urls = results_panel.xpath(
            ".//a[contains(@href,'CourtSummaryReport.ashx?docketNumber=')]"
        )
    ...
    async def fetch_cases_from_additional_page(
        ...
        # do the POST
        next_page, next_errs = await self.post(session, sslcontext, self.BASE_URL, data)
        errs += next_errs
    ...
    def get_docket_search_post_data(
        self, court, county, dkt_type, sequence, year, nonce, viewstate
    ):
        """
    ...
    async def search_docket_number(self, docket_number: str) -> Tuple[dict, List[str]]:
        """
        ...
        errs = []
        async with aiohttp.ClientSession(headers=headers) as session:
            search_page, search_page_errs = await self.fetch(
                session, sslcontext, self.BASE_URL
            )
            errs += search_page_errs
            ...
            search_results_page, results_page_errs = await self.post(
                session, sslcontext, self.BASE_URL, search_data
            )
            errs += results_page_errs
            if search_results_page == "":
                return dict(), errs

            search_results = self.search_results_from_page(search_results_page)
            return search_results, errs
